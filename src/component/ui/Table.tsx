            ...
            <thead className="text-white border-b  dark:text-black border-[#858585] ">
              <tr>
                {fieldsHeadings.map((item: string, ind: number) => (
                  <th className={`font-normal ${item==='Created At'?'hidden xl:block ':''} ${item==='Match Info'?'hidden xl:block ':''} ${item==='Status'?'hidden xl:block ':''} ${item==='Username'?'text-start':'text-center'} text-[.8rem] md:text-base pb-2`} key={ind}>
                    {item}
                  </th>
                ))}
              </tr>
                  ...
                  >
                    {fieldData.map((field, idx) => {
                      ...
                      switch (field) {
                        ...
                        case active:
                          return (
                            <td key={idx} className={`pt-4 ${field === "status" ? "hidden xl:inline-block": ""} pb-2`} >
                        ...
                        case date:
                          return (
                            <td key={idx} className={`pt-4  pb-2 ${date === "createdAt" ? "hidden xl:inline-block": ""}`}>
                              {formattedDate}
                            </td>
                          );
                        ...
                        case "receiver":
                          ...
                          case "match_info":
                          return (
                            <td key={idx} className="pt-4 pb-2 xl:inline-block hidden">
